...
- [无限地图_推进进度](tile-advanture-design/进度/无限地图_推进进度.md) — **✅ L1.2 整体闭环**（多源视野+据点+不 reload 昏迷复活）；**🔧 L1.3 进行中**（原拆 3 份，2026-06-08 增拆为 4 份）：**L1.3a ①「扎营时钟+胜负模型」整体闭环 ✅**（4 阶段 `ee74728`→`aa2d311`，桌面全验；扎营主时钟取代 cycle + climax sudden-death + 双失败 + 命数脱钩 + 同图同英雄 + cycle 退役 + 招募有界）；**L1.3b 无限化地基 🔧 实装中**（②③ 共同前置：渲染/移动切 chunk + `_schema` 退权威 + 战斗隔离；病根=`MapSchema.terrain_grid` 预分配全图数组；拍板甲方案=无界坐标+稀疏 chunk+多 VisionSource，否决乙滑动窗口；5 阶段 A→E）：**阶段 A 数据层 ✅** `3b47f0b`（地形权威收敛 ChunkPCG + MapSchema 双模无界化[病根 fix] + 局部出生校验）+ **B 渲染切视口 ✅** `999660f` + **C 移动切 chunk ✅** `5552b8f`（**玩家已可走出 32×32**，隐形墙消失；含负坐标点击 floori 修复）；阶段 D 实体[**实体表须可序列化**] / E 战斗 待续；**L1.3c ②敌方解耦+威胁 / L1.3d ③视野时钟+压力边界 待 L1.3b 落地后撰写**；储备 4 条设计候选（议题 9 暗影流变/10 多焦点/11 实体稀疏表/13 多据点=世界存档）；+ 关联 L0 入口 6 事件系统（预启动，招募真正归宿）
