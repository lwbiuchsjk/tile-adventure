...
- [无限地图_推进进度](tile-advanture-design/进度/无限地图_推进进度.md) — **🔧 L1.2 整体闭环**（Phase 0-4 全落地 `89d1ad4`/`ef48938`/`359049e`/`f894936`；多源视野+据点+不 reload 昏迷复活+开局预占领视野修复+codex 闭环+场景 3-10 桌面全验，设计 §12 实装回写）；**🔧 L1.3 设计中**（2026-06-05 拆 3 份子 MVP；**L1.3a ① 扎营时钟+胜负模型设计落盘 ✅**，待实装；②③ 待前份落地后撰写）+ 关联开新 L0 入口 6 事件系统（预启动）
