...
- [无限地图_推进进度](tile-advanture-design/进度/无限地图_推进进度.md) — **🔧 L1.3 进行中**：L1.2 + L1.3a/b/c + **L1.3d-1 暗影压力引擎已整体闭环**（2026-06-17，难度随扎营×视野递增 + cycle 时钟死代码清）；**下一步 = 撰写 L1.3d-2「暗影据点与压力呈现」设计**（③ 拆分下半：暗影据点萌发 + 拔点减压 + 压力前兆信号 + climax boss 强度细化与决战体验）。北极星 L1.3c §三图1；阶段细节见进度文档 / 看板。关联 L0 入口 6 事件系统（预启动）
