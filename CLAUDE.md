...
- [无限地图_推进进度](tile-advanture-design/进度/无限地图_推进进度.md) — **✅ L1.2 整体闭环**（多源视野+据点+不 reload 昏迷复活）；**🔧 L1.3 进行中**（拆 3 份子 MVP）：**L1.3a ①「扎营时钟+胜负模型」整体闭环 ✅**（设计 + 4 阶段实装 A `ee74728`/B `77953c0`/C `9d3024a`/D `515d1b4` + recruit 有界化 `aa2d311`，桌面全验；扎营主时钟取代 cycle + climax 决战 sudden-death + 双失败 + 命数脱钩 + 同图同英雄 + cycle 范式退役 + 招募有界 stopgap）；**②敌方解耦+威胁 / ③视野时钟+压力边界 设计预启动**（① 落地解除前置）⭐**接手 ②③ 首要待决**：当前地图玩法仍 32×32（chunk 只是底座未接管渲染/移动），"渲染/移动切 chunk 让地形真正外扩"是 L1.1 标的 L1.3 议题但未拆进 ②③——见进度文档 §接手必读 / memory；+ 关联 L0 入口 6 事件系统（预启动，招募真正归宿）
