...
- 敌方援军（L1.4）— 入口 5，**L1.1-1.3 已落地，具备启动条件**；设计要点全拍板见 [[持久slot战场参与设计#十、敌方援军（预启动 · L1.3 后启动）]]（归属透明储备 + config 默认静默 + 纯增量 4 步）；启动时设计先行写 `敌方援军_MVP.md`，并入**品质显式**（待跟踪索引 P2，战场单位格品质角标/色，与敌方援军条带变色同属战场视觉）

## 已归档

- [持久slot战场参与_推进进度](tile-advanture-design/进度/持久slot战场参与_推进进度.md) — 入口 5；3 份 L1 全落地（撤离 394cc0c / 援军 f7b1fcb·03ac4f5 / 周期胜利 b880656，2026-05-21 桌面验收）；延伸 L1.4 敌方援军见预启动区、品质显式登记待跟踪 P2
