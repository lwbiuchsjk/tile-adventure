...
## 格式（M8 v2：全局唯一）：
##   村庄 / 城镇：`类型名 + 全局序号`（如 "村庄1" ~ "村庄18"）；玩家 / 敌方 / 中立**共享**编号序列
##   核心城镇：`核心`（MVP 只有 2 个，靠势力色区分）
## 由 PersistentSlotGenerator 在生成完成后按 (类型, position y→x) 确定性排序分配，
## 保证 seed 复现两次得到相同 ID；**归属翻转不改变 ID**，玩家可能拥有不连续的编号
## 未分配时为空串、UI 回退到 get_map_label
