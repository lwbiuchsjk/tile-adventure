...
extends Node
## 核心目标传达 L1.5（核心目标传达_MVP §2.3）：大地图边缘带 + 离屏敌方核心方向标记
##
## 设计原文：tile-advanture-design/核心目标传达_MVP.md §2.3
##
## 思路（桌面跑测三轮定稿，用户拍板）：不重构相机；在大地图 HUD 上方加一层"边缘带"——
## 屏幕四周一圈窄而实体的深色边框带（地图区 = 内缩矩形），离屏目标方向在带上画黄色光晕。
## **黄色光晕用 clip_children 遮罩裁到边框带内**——只在带内显式、不溢进地图区，醒目感 + 方向性兼得。
## 该带是可复用的"地图外指示区"：当前画敌方核心方向，日后可摆各类地标 ICON。
## 仅大地图态显示：战斗中相机 zoom 到战场、指引无意义 → 隐藏整层（set_battle_active）。
##
## 结构：本体 Node 持 CanvasLayer=7；内含 BandNode（Node2D，画边框带 + clip_children 裁剪）；
## BandNode 子 GlowNode（Node2D，画黄色光晕 → 被裁到边框带内）。
...
## 黄色光晕：找最近敌方核心 → 投影 → 地图区内则不画 → 否则在带中线画光晕（被 clip 裁到带内）
func draw_glow(canvas: Node2D) -> void:
	if _camera == null:
		return
	var target_world: Vector2 = _find_nearest_enemy_core_world()
	if target_world.x < 0.0:           # 无敌方核心（已占领 / 未生成）→ 不画
		return
	var fr: Rect2 = _frame_rect(canvas)
	var w: float = _band_w(fr.size)
	...
	if map_area.has_point(screen_pt):  # 核心在地图区内 → 隐藏（改动 2 高亮接管）
		return
	...
	canvas.draw_circle(at, marker_glow_radius, marker_glow_color)
	canvas.draw_circle(at, marker_glow_radius * 0.6, marker_glow_color)
	canvas.draw_circle(at, marker_core_radius, marker_core_color)
...
## 找最近的 ENEMY_1 owned CORE_TOWN 世界像素中心；无则返回 (-1,-1)
func _find_nearest_enemy_core_world() -> Vector2:
	if _world_view == null:
		return Vector2(-1.0, -1.0)
	var schema: MapSchema = _world_view.get_schema()
	if schema == null:
		return Vector2(-1.0, -1.0)
	var cam_center: Vector2 = _camera.get_screen_center_position() if _camera != null else Vector2.ZERO
	...
	for entry in schema.persistent_slots:
		...
		if slot == null:
			continue
		if slot.type != PersistentSlot.Type.CORE_TOWN or slot.owner_faction != Faction.ENEMY_1:
			continue
		var world_c: Vector2 = _world_view.grid_to_pixel_center(slot.position)
		var d: float = world_c.distance_squared_to(cam_center)
		if d < best_d:
			best_d = d
			best = world_c
	return best
