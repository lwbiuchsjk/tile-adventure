...
func setup(player_cfg: PlayerParamResource, run_cfg: RunParamResource) -> void:
	...
	# L1.3a 阶段 D：cycle 范式退役——run 内已无 reload，setup 仅在开局调一次。
	# 队长固定走 draw_new_leader（设计 §2.5：draw_new_leader 仅开局抽一次初始队长）；
	# 原"周期胜利重建队长"快照分支随 advance_cycle_on_victory 一并移除。
	...
	# 单角色：队长占据 _characters[0]，其余空位由入队事件追加
	var ch: CharacterData = CharacterData.new()
	ch.id = 1
	# 写入 hero_id，让 draw_recruit 能正确排除当前在队英雄
	...
	# 重生叙事占位（昏迷 reload 范式遗留）：阶段 D 后 advance_cycle 退役、_pending_respawn_intro 已无人置位，
	# 分支冻结保留（不级联清理 respawn_intro_ready 信号线）。开局命中即 emit 让 WorldMap 解除过渡 await。
